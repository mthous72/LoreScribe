...
| [Drizzle — proxy driver](https://orm.drizzle.team/docs/sqlite/connect-drizzle-proxy) · [custom migrations](https://orm.drizzle.team/docs/kit-custom-migrations) | No sqlite-wasm entry exists; `sqlite-proxy` is the path; the callback returns **positional arrays**, not row objects; the proxy migrator is Node-only. *Drizzle itself was removed in [D28](10-decisions.md); these findings stay because they are why `SqlDriver` has the shape it does* |
